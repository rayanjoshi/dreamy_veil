...
class DataAnalysis:
    ...
    def visualise_yield_curves(self):
        ...
        fig.add_trace(
            go.Scatter(x=data.index, y=data["10Y-2Y Treasury Yield Spread"], name = "US 10Y-2Y Spread"),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Scatter(x=data.index, y=data["UK Bond Yield Spread"], name = "UK 10Y-3M Spread"), row=1, col=2
        )
    ...
    def market_analysis(self):
        ...
        X = sm.add_constant(data[["US_Spread_Change", "UK_Spread_Change",
                                "US_Rate_Change", "UK_Rate_Change"]])
        
        y_us = data["US_Bond_Returns"]
        y_uk = data["UK_Bond_Returns"]
        
        model_us = sm.OLS(y_us, X).fit()
        print("US Bond Returns regression:")
        print(model_us.summary())

        model_uk = sm.OLS(y_uk, X).fit()
        print("UK Bond Returns regression:")
        print(model_uk.summary())
    
    def compare_bond_reactions(self):
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data.index, y=data["US_Cum_Return"],
                                name="US AGG (cum return)"))
        fig.add_trace(go.Scatter(x=data.index, y=data["UK_Cum_Return"],
